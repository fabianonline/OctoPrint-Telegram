...
############################################################################################
        def cmdPrinterOn(self,chat_id,from_id,cmd,parameter):
                if self.main._plugin_manager.get_plugin("tplinksmartplug",True):
                        try:    #Let's check if the printer has been turned on before.
                                ...
                                answer = requests.get("http://localhost:" + str(self.port) + "/api/plugin/tplinksmartplug?checkStatus=" + self.main._settings.get(['tplink_ip_address']),  json={}, headers=headers)
                ...
                if self.main._plugin_manager.get_plugin("tplinksmartplug",True):
                        try:    #Let's check if the printer has been turned off before.
                                ...
                                if (answer.status_code >= 300):
                                        ...
                                        if (answer.json()["currentState"] == 'off'):
...
############################################################################################
        def cmdSwitchOff(self,chat_id,from_id,cmd,parameter):
                ...
                try:
                        headers = {'Content-Type': 'application/json', 'X-Api-Key' : self.main._settings.global_get(['api','key'])}
                        postData = '{"command":"turnOff", "ip":"' + self.main._settings.get(['tplink_ip_address']) + '"}'
                        answer = requests.post("http://localhost:" + str(self.port) + "/api/plugin/tplinksmartplug", data=postData, headers=headers)
                        if (answer.status_code  >= 300):
                                self._logger.debug("Call response (POST API octoprint): %s" % str(answer))
...
############################################################################################
        def cmdSwitchOn(self,chat_id,from_id,cmd,parameter):
                ...
                try:
                        headers = {'Content-Type': 'application/json', 'X-Api-Key' : self.main._settings.global_get(['api','key'])}
                        postData = '{"command":"turnOn", "ip":"' + self.main._settings.get(['tplink_ip_address']) + '"}'
                        answer = requests.post("http://localhost:" + str(self.port) + "/api/plugin/tplinksmartplug", data=postData, headers=headers)
                        if (answer.status_code >= 300):
                                self._logger.debug("Call response (POST API octoprint): %s" % str(answer))
